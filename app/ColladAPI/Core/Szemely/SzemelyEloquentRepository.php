...
class SzemelyEloquentRepository extends RestExtRepository implements SzemelyRepository {
    ...
    public function findTanarokAtTime(\DateTime $atTime)
    {
        ...
        return $res->restCollection($this->pagination);
    }
    ...
    {
        return $this->entity->with('fokozatok')->where('id', '=', $id)->firstOrFail()->getRelation('fokozatok');
    }
    ...
    public function tanszekekForSzemely($id)
    {
        ...
        return $this->entity->with('tanszekek')->where('id', '=', $id)->firstOrFail()->getRelation('tanszekek');
    }
}